...
import { Plus, ArrowLeft, LogOut, Trash2, Search, X, Package, Settings, Menu as MenuIcon, Tag, BarChart2, Smile, ChevronDown, ChevronUp, PenSquare, ChevronLeft, Route, ExternalLink, ChefHat, Utensils, DollarSign, MessageCircle, } from 'lucide-react';
...
const SIDEBAR_TABS = [
    ...
    { id: 'whatsapp', label: 'Atendente', icon: MessageCircle },
      ...
      if (currentCount > previousNewOrdersCount.current) {
        ...
        if ('Notification' in window && Notification.permission === 'granted') {
          new Notification('🍕 Novo Pedido Recebido!', {
            ...
            icon: '/jatai.png', // Garante que a notificação use a imagem da pasta public
            tag: 'new-order'
          });
        }
      }
  ...
  return (
    <div className="flex h-screen bg-gray-100" style={{ fontFamily: theme.fontFamily }}>
      ...
      <div className={`fixed lg:relative bg-gray-800 text-white flex flex-col flex-shrink-0 transition-transform duration-300 ease-in-out z-30 h-full ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 ${isSidebarCollapsed ? 'w-20' : 'w-64'}`}>
        <div className="flex flex-col items-center justify-center h-56 border-b border-gray-700">
          <img src="/jatai.png" alt="Jataí Food" className={`h-48 ${isSidebarCollapsed ? 'hidden' : ''}`} />
          <img src="/jatai.png" alt="Jataí Food" className={`h-44 ${isSidebarCollapsed ? '' : 'hidden'}`} />
          {!isSidebarCollapsed && <span className="mt-2 text-sm font-medium text-white">{authUsername}</span>}
        </div>
        ...
        <div className="px-4 py-4 border-t border-gray-700 z-10">
            <button 
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white shadow-md flex items-center">
          <button className="hidden lg:block text-gray-600 p-4" onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)}>
              <MenuIcon className="h-6 w-6" {...iconProps} />
          </button>
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          <div className={activeTab === 'whatsapp' ? '' : 'hidden'}><WhatsAppAttendanceSection /></div>
